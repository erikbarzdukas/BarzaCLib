...
static inline void String_setup_skip_chars(
        size_t *skip_chars,
        const unsigned shar *needle, ssize_t nlen) {
...
static inline const unsigned char *String_base_search(
    ...
    check(nlen > 0, "needle length can't be <= 0");
    check(hlen > 0, "haystack length can't be <= 0");
...
int String_find(bstring in, bstring what) { 
    ...
    String_setup_skip_search(skip_chars, needle, nlen);
